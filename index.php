...
$max_price = !empty($prices) ? ceil(max($prices)) : 500;
?>
<script>
        ...
        <div class="col-md-3">
            <div id="product-filter" class="container mb-4">
                <form id="ajax-filter-form">
                                ...
                                <div class="accordion-group">
                                    ...
                                    <div class="fil-body open">
                                        <div class="price-inputs my-3 d-flex">
                                            ...
                                            <input type="number" id="max_price" name="max_price" class="form-control d-inline w-50 " value="400">
                                        </div>
                  ...
                  <label class="form-check-label custom-checkbox" for="cat_' . $brand->term_id . '"><input class="form-check-input" type="checkbox" name="sub_cat[]" value="' . esc_attr($brand->slug) . '" id="cat_' . $brand->term_id . '"><span class="checkmark"></span>' . esc_html($brand->name) . ' (' . $count . ')</label>
                </div>';
                                                }
                                                ?>
                        ...
                        foreach ($all_attrs as $attr) :
                            ...
                            $current_cat = get_queried_object();
                            if ($current_cat->slug == 'avalyne' && $taxonomy == 'pa_dydis') {
                                continue;
                            }
                            // For hide filter item
                            $current_cat = get_queried_object();
                            if ($current_cat->slug == 'rakeciu-aksesuarai' && $taxonomy == 'pa_storis-mm') {
                                continue;
                            }
                            // For hide filter item
                            $current_cat = get_queried_object();
                            if ($current_cat->slug == 'krepsiai-ir-kuprines' && $taxonomy == 'pa_talpa-l') {
                                continue;
                            }
                            ...
                            $current_cat = get_queried_object();
                            if ($current_cat->slug == 'raketes-stalo-teniso-raketes' && in_array($taxonomy, $exclude_taxonomies3, true)) {
                                continue;
                            }
        ...
        <div class="col-md-9">
            <div class="cat_details">
                ...
                if (!empty($term->description)) {
                    echo '<div class="term-description">' . term_description($term) . '</div>';
                }
                ?>
            </div>
        </div>
